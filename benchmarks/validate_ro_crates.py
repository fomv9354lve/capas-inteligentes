...
def main() -> int:
    ...
    for trace_id, (coverage, status, evidence_expected, action_status) in EXPECTED.items():
        try:
            ...
            assert "workflow:capas-costurero-run" in root_parts, "workflow not listed as crate part"
            ...
            assert action.get("instrument", {}).get("@id") == "workflow:capas-costurero-run", "run missing workflow instrument"
