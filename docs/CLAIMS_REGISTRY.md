...
| pip install capas-claim-gate | Home / README install CTA | **BACKED** | `pip install capas-claim-gate (PyPI; published from GitHub via OIDC Trusted Publishing, no token)` | Published via the publish.yml Trusted Publisher (OIDC, triggered on GitHub release); capas-claim-gate 0.3.0 is live on PyPI (0.4.0 local, pending). |
