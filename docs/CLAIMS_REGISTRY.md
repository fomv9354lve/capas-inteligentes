...
| pip install capas-claim-gate | Home / README install CTA | **SCOPED** | wheel built + twine-check PASSED; NOT yet published to PyPI | The install command works only AFTER `twine upload`. Until then the homepage CTA must not imply it is live. **Upgrades with:** twine upload (needs the maintainer's PyPI token). |

**Ledger:** 3 CLOSED · 3 BACKED · 4 SCOPED · 0 bare.
