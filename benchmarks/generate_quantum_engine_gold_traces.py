...
TRACE_SPECS = [
    ...
    ("trace_015", "quimb_mps_certified_bound_candidate", {"n": 60, "depth": 6, "max_bond": 8, "seed": 1}, "certified_bound_candidate"),
]
...
def main() -> None:
    ...
    for trace_id, fn, kwargs, coverage_case in TRACE_SPECS:
        ...
        if coverage_case not in {"backend_failed", "no_evidence_success", "certified_bound_candidate"}:
        ...
        if physical_level in {"none", "certified_bound"}:
        ...
        audit_rows.append({
            ...
            "inference_blind_judge": "hold" if physical_level in {"analytic", "cross_sim", "certified_bound"} else "no",
            "inference_correct": "unknown" if physical_level in {"analytic", "cross_sim", "certified_bound"} else "no",
            ...
            "coverage_case": coverage_case,
            "risk_level": "low",
            "decision": "hold" if physical_level in {"analytic", "cross_sim", "certified_bound"} else "reject",
            "notes": (
                ...
                else "Candidate certified-bound trace; needs independent review before being treated as evidence."
                if physical_level == "certified_bound"
